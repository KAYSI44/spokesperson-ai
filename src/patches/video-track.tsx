...
export interface VideoTrackProps
  ...
  mediaEl: React.RefObject<HTMLVideoElement | undefined>;
}
...
export function VideoTrack({
  ...
  const _name =
    ...
    maybeTrackRef?.publication?.trackName ??
    name;
  ...
  if (_source === undefined) {
    throw new Error(
      'VideoTrack: You must provide a trackRef or source property.',
    );
  }

  const participant = useEnsureParticipant(_participant);

  const intersectionEntry = useHooks.useIntersectionObserver(mediaEl as React.RefObject<HTMLVideoElement>, {});

  const debouncedIntersectionEntry = useHooks.useDebounce(
    intersectionEntry,
    3000,
  );
  ...
  const {
    ...
    {
      element: mediaEl as React.RefObject<HTMLVideoElement>,
      props,
    },
  );
  ...
  return (
    <video
      ref={mediaEl as React.RefObject<HTMLVideoElement>}
      {...elementProps}
      muted={true}
